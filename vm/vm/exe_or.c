...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/11/24 21:00:18 by vzomber           #+#    #+#             */
/*   Updated: 2018/11/24 21:00:32 by vzomber          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "vm.h"

void	or_logs(t_tool *tool, t_carriage *carg)
{
...
int		second_param_or(t_carriage *carg, t_tool *tool, int av2, int av1)
{
	int	num2;
	...
	carg->tmp = 0;
	if (av2 == IND)
		num2 = (unsigned int)get_ind_value(tool, carg->curr_place, av1);
	else if (av2 == DIR)
...
int		first_param_or(t_carriage *carg, t_tool *tool, int av1)
{
	int	num;
	...
	if (av1 == IND)
		...
		num = (unsigned int)get_value(tool, carg->curr_place, DIR);
	else if (av1 == REG)
	{
...
void	second_part_or(t_tool *tool, t_carriage *carg, int len, int reg)
{
...
void	exe_or(t_carriage *carg, t_tool *tool, int cycles)
{
	...
	num1 = first_param_or(carg, tool, av1);
	num2 = second_param_or(carg, tool, av2, av1);
	...
	second_part_or(tool, carg, (2 + av1 + av2 + REG), av3);
}
